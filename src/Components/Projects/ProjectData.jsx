// ProjectData.jsx
import netflix from '../../assets/banner/netflix.png'
import crypto from '../../assets/banner/crypto.png'
import mern from '../../assets/banner/mern.png'
import shoppy from '../../assets/banner/shoppy.png'
import todo from '../../assets/banner/todo.png'
import college from '../../assets/banner/college.png'
import zwiggy from '../../assets/banner/zwiggy_image.png'
import productList from '../../assets/banner/poductList.png'
import useraddress from '../../assets/banner/UserAddress.png'
import ecommerce from '../../assets/banner/ecommerce.png'
import chatflow from '../../assets/banner/chatflow.png'
import moneyManager from '../../assets/banner/moneyManager.jpg'

const ProjectData = [
  {
      id: 1,
      title: "Money Manager",
      description: "The Money Manager is a responsive full-stack web app for tracking and managing income and expenses It allows adding, editing, deleting transactions, and viewing trends via interactive charts.Users can download or email detailed financial reports for easy record-keeping.Tech Stack: React.js, Tailwind CSS, Spring Boot, MySQL, Recharts",
      liveLink: "https://moneymanager-rbhm.onrender.com/",
      githubLink: "https://github.com/PraveenNagaraj77/MoneyManager-frontend",
      image: moneyManager, // Add the URL to the Netflix banner image
      tags: ["Fullstack"]
    },
    {
      id: 2,
      title: "E-Commerce Shopping",
      description: "The Ecommerce Application is a full-featured web application that allows users to browse products, add items to the cart, place orders, and manage their profiles. Admins can manage products, categories, orders, and users through an intuitive dashboard. ",
      liveLink: "https://e-commercebypraveen.onrender.com/",
      githubLink: "https://github.com/PraveenNagaraj77/E-Commerce-Application",
      image: ecommerce, // Add the URL to the Netflix banner image
      tags: ["MERN Stack"]
    },
{
      id: 3,
      title: "Food Delivery-Zwiggy",
      description: "Developed Using MERN Stack , We can order food, Select Quantity, make payment,View Orders and in admin panel we can update food details ,Data is stored in MongoDB ",
      liveLink: "https://food-delivery-frontend-iqr6.onrender.com/",
      githubLink: "https://github.com/PraveenNagaraj77/Food-Delivery",
      image: zwiggy, // Add the URL to the Netflix banner image
      tags: ["MERN Stack"]
    },
    {
      id: 4,
      title: "Product Listing",
      description: "Developed a responsive web application using NextJs. Where Data is fetched from an  API, we can List all  Products Filter By Category & Search products",
      liveLink: "https://product-listing-sand.vercel.app/",
      githubLink: "https://github.com/PraveenNagaraj77/ProductListing",
      image: productList, // Add the URL to the Netflix banner image
      tags: [""]
    },
    {
      id: 5,
      title: "Netflix Clone",
      description: "Developed a responsive web application using React.js. Integrated with the TMDB API for video content. Implemented authentication using Firebase.",
      liveLink: "https://net-flixx-clone-bypraveen.netlify.app/",
      githubLink: "https://github.com/PraveenNagaraj77/NetFlix-Clone",
      image: netflix, // Add the URL to the Netflix banner image
      tags: ["MERN Stack"]
    },
    {
      id: 6,
      title: "Crypto Price Tracker",
      description: "Built a React.js application to display the top 10 cryptocurrencies. Allowed users to search and view individual cryptocurrency data through an integrated API.",
      liveLink: "https://cryptoprice-track-praveen.netlify.app/",
      githubLink: "https://github.com/PraveenNagaraj77/CryptoPriceTracker",
      image: crypto, // Add the URL to the Crypto Price Tracker banner image
      tags: [""]
    },
    {
      id: 7,
      title: "User Address Registration",
      description: "The User Address Registration project, built with the MERN stack, enables users to register, view, and delete their addresses. It provides a simple interface for efficient address management",
      liveLink: "https://useraddressregistration-frontend.onrender.com/",
      githubLink: "https://github.com/PraveenNagaraj77/User-Address-Registration",
      image: useraddress, // Add the URL to the MERN CRUD App banner image
      tags: ["MERN Stack"]
    },
    {
      id: 8,
      title: "MERN CRUD App",
      description: "Created a CRUD application using the MERN stack. Enabled users to add, update, and delete posts with data stored in MongoDB.",
      liveLink: "https://merncrudapp-te7v.onrender.com/",
      githubLink: "https://github.com/PraveenNagaraj77/MERNCruddApp",
      image: mern, // Add the URL to the MERN CRUD App banner image
      tags: ["MERN Stack"]
    },
    {
      id: 9,
      title: "ShoppyDay E-Commerce Website",
      description: "Developed a responsive e-commerce website with modules for selecting products and categories (men, women, kids). Added features to add products to the cart and generate the total amount. Built with HTML, CSS, JavaScript, and React.js. Backend integration planned for future updates.",
      liveLink: "https://shoppyday.netlify.app/",
      githubLink: "https://github.com/PraveenNagaraj77/Shoppyday",
      image: shoppy, // Add the URL to the ShoppyDay banner image
      tags: ["MERN Stack"]
    },
    {
      id: 10,
      title: "ToDo-List App",
      description: "Adds task list, update task list, delete task list.",
      liveLink: "https://todolist-app-react7.netlify.app/",
      githubLink: "https://github.com/PraveenNagaraj77/TodoListApp",
      image: todo, // Add the URL to the ToDo-List App banner image
      tags: [""]
    },
    {
      id: 11,
      title: "Responsive College Website",
      description: "Simple Website using HTML, CSS, JS with Responsive Design.",
      liveLink: "https://college-page.netlify.app/",
      githubLink: "https://github.com/PraveenNagaraj77/Responsivewebsite",
      image: college, // Add the URL to the Responsive College Website banner image
      tags: [""]
    },
    {
      id: 12,
      title: "Chatflow Builder",
      description: "A visual interface for designing chatbot conversation flows—essentially a no-code tool where you can create, connect, and edit dialog nodes to build chat logic. It lets you drag and drop message/choice nodes, link them together to define conversation paths, and easily adjust flow behavior  ",
      liveLink: "https://chat-bot-flow-builder-orpin.vercel.app/",
      githubLink: "https://github.com/PraveenNagaraj77/ChatBot-Flow-Builder",
      image: chatflow, // Add the URL to the Netflix banner image
      tags: [""]
    },
  ];
  
  export default ProjectData;
  
